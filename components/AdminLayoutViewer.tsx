...
interface AdminLayoutViewerProps {
  refreshId: string;
  viewToken: string;
  layouts: LayoutItem[];
}

export function AdminLayoutViewer({ refreshId, viewToken, layouts }: AdminLayoutViewerProps) {
    ...
    <>
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        ...
        <div className="mt-0">
          ...
          {currentLayout.rationale && (
            <details className="mt-3 rounded-lg border border-border bg-muted/20 px-4 py-2">
              <summary className="cursor-pointer text-sm font-medium text-muted-foreground">
                Agent Rationale
              </summary>
              <p className="mt-2 text-sm whitespace-pre-wrap">{currentLayout.rationale}</p>
            </details>
          )}
